...
//! USB devices matching the touchy-pad VID/PID *and* the
...
impl DiscoveredDevice {
	...
	/// physical connection); the simulator keys off `host:port`. This
	/// is **not** the device serial — callers that want the
	/// hardware serial must [`open`][Self::open] and query
	/// `sys_board_info_get`.
...
///
/// Includes USB devices matching the touchy-pad VID/PID and — when
/// `TOUCHY_SIM_URL` is set — the Python simulator. The simulator entry
/// is appended last so USB hardware takes precedence when a caller
/// just wants "the first device". A failed USB enumeration is treated
/// as "no USB devices" (logged) rather than an error, so the simulator
/// remains discoverable on hosts without libusb.
