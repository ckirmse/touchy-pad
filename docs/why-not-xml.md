...
- Encoded screens are written to LittleFS at `/littlefs/host/s/<name>.pb` (drive-prefixed `F:host/s/<name>.pb` on the wire) by the existing host-api file-upload path (stage 13, refined in stage 51 to use the streaming-write protocol; the directory moved from `host/screens/` to `host/s/` in stage 68). No new transport.
...
- On boot, scan `/littlefs/host/s/*.pb` (and the analogous PSRAM `R:host/s/`), decode each with nanopb, build an in-memory `std::map<std::string, std::string>` keyed by full drive-prefixed path.
